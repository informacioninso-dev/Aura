    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#94A3B8] text-sm mt-1">Total mensual estimado: <span className="text-[#10B981] font-semibold">${total.toLocaleString('es-CL', { maximumFractionDigits: 0 })}</span></p>
        </div>
            ...
            <tbody>
              {items.map(item => (
                <tr key={item.id} className="border-b border-[#334155]/50 hover:bg-[#334155]/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-[#10B981] font-semibold text-sm">${parseFloat(item.monto).toLocaleString('es-CL')}</td>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input required value={form.descripcion} onChange={e => setForm({ ...form, descripcion: e.target.value })}
              className="w-full bg-[#0F172A] border border-[#334155] rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-[#10B981]" placeholder="Ej: Sueldo mensual" />
          </div>
